...
if __name__ == "__main__":
    ...
    for my_storage, i in yt.parallel_objects(range(len(sub_dict_list)), storage=storage, dynamic=False):
        ...
        if (len(subh_lums) < 4) or (len(gal_lums_hosts[host_idx]) == 0):
            ...
            bgg_4th = np.sort(subh_lums)[-3]
            bgg = gal_lums_hosts[host_idx]
            
            #if BCG < 4th-BCG
            if bgg < bgg_4th:
                bgg = np.sort(subh_lums)[-1]
                bgg_4th = np.sort(subh_lums)[-4]
                ...
                bgg_id = sub_ids[np.argsort(subh_lums)][-1]
    ...
    if yt.is_root():
        ...
        yt.save_as_dataset(fake_ds, 'r_mag_gap/m8_1800_z0_500c_1e13_50kpc.h5', data= data) 
